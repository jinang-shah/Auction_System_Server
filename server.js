...
// app.use(express.static("/home/krunal/Public/Auction_System_Server/images"))
app.use(express.static("/home/krunal/Public/Auction_System_Server/"))
app.use(express.static("/home/krunal/Public/Auction_System_Server/documents"))
...
cron.schedule("20 19 * * *", async function() {
    ...
    try {
        await Product_Model.updateMany({ startDate: { $lte: currentDate } }, { $set: { status: "live" } });
        await Product_Model.updateMany({ endDate: { $lte: currentDate } }, { $set: { status: "completed" } });
